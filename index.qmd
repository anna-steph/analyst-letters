...
youth_data <- pull_fred_series(api_key = params$fred_api_key, 
                               series_ids = youth_series) %>%
  rename(date = obs_date)

data_updated <- youth_data %>%
  select(updated_at) %>%
  unique() %>%
  pull(max(updated_at)) %>%
  format("%B %d, %Y")
...
> **As we again face future waves of labor market uncertainty, I know we can do better.**
...
to help them up the first rungs. No one does this alone, but early career folks 
may not know where to start. 

We can also do better by mid-career analysts seeking to level up, eager
to learn but too shy or proud to ask where to start. The labor market's always
evolving, a kaleidoscope dance floor we trip to keep pace with, a path of laser
fields we navigate haphazardly, learning through our stumbles. Continuing education
in most fields is neither easy nor obvious.
